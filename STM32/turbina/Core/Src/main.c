...
//	  HAL_Delay(2000);
//	  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, (uint16_t) 50);
  }
  /* USER CODE END 3 */
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim == &htim3)
	{
		...
		//__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, (uint16_t)motor_calculate_speed(&motorA));
	}
}
/* USER CODE END 4 */
